...
import { addCharacter, addCharacterAxios } from '@services/services'
...
const NewCharacter = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // const response = await addCharacter( dataToSend )
      const response = await addCharacterAxios( dataToSend )
      console.log(response)


      if ((response.status >= 200) && (response.status < 300)) {
        // const result = await response.json();
        // const result = await response.data;  // format axios - la resposta ja porta implicita la conversió a .json() i no cal fer-la. La informació ésta a .data
        ...
        setMessage('Error en enviar les dades');
      }
    } catch (error) {
  ...
  return (
    <div style={styles.container}>
      <h2 style={styles.title}>Crear Nou Personatge</h2>
